...
4. In OnServe hook (the schema already exists at this point), in this order:
   ...
   - Install the **authz adapter**: `authzpb.NewDeps(app, scrMgr, prefixFn)` → `authzpb.SetDefault` + `svc.Authz` (before the seeder, so hooks firing during seeding decide through it — a nil default denies), then `authzpb.ImportLegacyEnv` (PD-12 `LAN_SAVES_TOKEN` → in-memory "legacy-env" machine key)
