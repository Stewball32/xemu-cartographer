...
**Symmetric teardown (per-instance vs shared).** `Manager.Remove` → `removeContainerFiles` ([podman.go](internal/podman/podman.go)) removes *every* per-instance file Create generated — the overlay (`hdds/<name>.qcow2`), the xemu config dir (`configs/<name>/`, which holds the **per-instance eeprom** at `/config/.local/share/xemu/xemu/eeprom.bin`, plus toml/ssl/shaders/X state) and the browser profile (`browser/config-<name>/`). It never touches **shared** files: the root qcow2 (+ any `_`-prefixed baseline), the bootrom/MCPX + flashrom/BIOS under `shared/bios/`, the default toml. `DeleteFiles` reuses the same remover (after a running-check); `containerOwnedPaths` is the single source of truth for "per-instance".
