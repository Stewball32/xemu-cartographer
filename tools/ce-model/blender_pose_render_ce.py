...
# CE/H1 cyborg bones are "bip01 l thigh"; normalize to the UE/MCC convention
# ("l_thigh") so the cross-game pose retarget name-matches. CE is a name-subset
# of the UE rig (no fingers/toes/weapon) — unmatched UE bones simply don't pose.
def _norm(n):
    n = n.strip()
    return n[6:].replace(" ", "_") if n.startswith("bip01 ") else n.replace(" ", "_")
names = [_norm(str(x)) for x in d["node_names"]]; parent = d["node_parent"]
...
diff_img = img(os.path.join(TEX, "cyborg.png"))
def mat_diffuse():
...
# CE tints by a SINGLE armor colour over the change-colour mask (multipurpose
# blue channel, baked to cyborg_cc.png). No primary/secondary, no emblem.
M_DIFF = mat_diffuse(); M_C = mat_mask(os.path.join(TEX, "cyborg_cc.png"))
...
for label in want:
    Pq, Pp = pose_world(label); pc, pn = skin(Pq, Pp)
    apply_coords(pc, pn)
    if not cam_set: setup_cam(); cam_set = True
    mesh.materials[0] = M_DIFF; render_to(os.path.join(OUT, f"{label}.png"))
    mesh.materials[0] = M_C;    render_to(os.path.join(OUT, f"{label}_c.png"))
