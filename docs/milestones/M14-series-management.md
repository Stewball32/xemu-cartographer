...
- 2026-06-18: **14d series-completion wiring landed** (as part of the M13 game-end chain). `internal/games.PersistFinishedGame` now calls `advanceSeries`, which tallies the series' games' `winner_team`, runs `series.Progress`, and stamps `ended_at` when the format is satisfied; the `Standing` is returned in the persistence `Result`. Integration-tested (a best-of-3 completes at 2-0 with `ended_at` set). Still deferred: attaching a finished game to a *pre-existing* active series by container/gamertag match (the writer takes an explicit `SeriesID` today; auto-create otherwise), and the 14a–c UIs. See the M13 Log (2026-06-18) for the full chain.
