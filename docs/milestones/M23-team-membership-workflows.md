...
- 2026-06-02: 23c — team_log + team_membership_requests + invite/join-request routes + public pages. Two new collections registered through `schema/identity.go` after rosters: `team_log` (team-centric activity timeline; `(team, created DESC)` + `(team, event, created DESC)` + `(subject_user, created DESC)` indexes; authed list/view; nil mutate) and `team_membership_requests` (the directed pending state; `(team, user, status)` + `(user, status, created DESC)` indexes; list/view = recipient OR initiator OR admin; nil mutate). `team_membership_requests` has no `gamertag` column — per the M23 default-gamertag decision, the accept route resolves it server-side at acceptance time. New `internal/teamlog` package with `Write(app, team, actor, event, subjectUser, subjectGamertag, payload)` helper + 11 per-event files (member_added/left/removed, invite_created/declined/cancelled, request_created/declined/cancelled, team_renamed, team_status_changed) following the audit/notifications convention. New `internal/teamperms` package: `IsOwnerOrManager`, `HasActiveMembership`, `OwnersAndManagers`, `FindPendingRequest` — walked from routes + hooks without an interface boundary (scraper + Discord don't need team perms). New `/api/teams/` route group bound with RequireAuth: `POST /api/teams/{teamId}/invite` (owner/manager only; rejects blocked teams, soft-deleted targets, targets with no usable default_gamertag, duplicate active membership, duplicate pending request; fires TeamInvite notification + EventInviteCreated team_log row) and `POST /api/teams/{teamId}/join-requests` (any authed user with a usable default; fans out TeamJoinRequest notifications to every active owner/manager + EventRequestCreated team_log row). `teams_status_transitions` hook extended to twin-write EventTeamRenamed + EventTeamStatusChanged to team_log alongside the existing audit_log calls so the public team page can render the timeline without admin-only audit_log access. Public pages: `/teams/[slug]/` replaces the placeholder (header + roster + former members + activity timeline reading `team_log` filtered to member_added/left/removed + team_renamed + Request to join CTA gated on auth + not-in-roster + no-pending-request); `/u/[username]/` replaces the placeholder body (header + bio + visible gamertags + current + past memberships + Invite-to-team picker visible to viewers who own/manage at least one team). Accept/decline/cancel + leave/remove routes land in 23d.
- 2026-06-02: 23d — accept / decline / cancel + self-leave + owner-remove. Two new route groups bound with RequireAuth: `/api/team-membership-requests/` (`accept.go` / `decline.go` / `cancel.go`) and `/api/rosters/` (`leave.go` / `remove.go`). Accept route per direction: invitee accepts for `invited`, owner/manager approves for `requested`; both resolve the recipient's current `default_gamertag` server-side (per the M23 social-handle decision — no UI picker), create a `rosters` row with `joined_at=now` + `is_owner=false` + `is_manager=false`, flip the request to `accepted` with `responded_by=auth`, notify the other party with the matching `*_accepted` payload, and emit one `EventMemberAdded` team_log row with a `via` payload field disambiguating the path. Decline route mirrors with status=`declined` + the matching `*_declined` notification + `EventInviteDeclined`/`EventRequestDeclined`. Cancel route is initiator-only (status=`cancelled`); for invite-direction it notifies the recipient with `*_cancelled`, for request-direction it fans out one notification per owner/manager. Leave route is self-only (caller must own the gamertag on the row), stamps `left_at=now`, writes `EventMemberLeft`, no notification fan-out. Remove route is owner/manager-only (caller is owner/manager of the team AND not the gamertag owner — use leave for self), stamps `left_at=now`, notifies the removed user with `team_removed`, writes `EventMemberRemoved`. Frontend wiring: new `membership-actions.ts` helper centralizes the seven POST endpoints so the bell + page + settings + team page share one fetch+error shape; `notification-render.ts` gains `isActionableInvite` + `extractRequestID` helpers so the actionable rows know which `request_id` the inline Accept/Decline buttons should target. `HeaderBell.svelte` + `/notifications/+page.svelte` render Accept/Decline buttons inline on unread `team_invite` + `team_join_request` rows. `/teams/[slug]/+page.svelte` adds per-row Leave (visible to the gamertag owner on their own active row) and Remove (visible to owner/manager on others' rows) buttons, both gated on a Skeleton `confirmToast`. New `PendingRequests.svelte` component renders the caller's incoming invitations + outgoing pending in the `/settings/` Gamertags & Teams section so users have a non-bell view of pending state. Backend builds + `pnpm check / lint / test` clean.
