...
test('debug-previous-game: /pod/debug/[name]/#previous_game renders envelope-class tab', async ({
	page
}) => {
	await installPodMocks(page);
	await loginAsAdmin(page);

	await page.goto(`/pod/debug/${MOCK_INSTANCE_LIVE}/#previous_game`);
	...
	await screenshotPodRoute(
		page,
		`/pod/debug/${MOCK_INSTANCE_LIVE}/#previous_game`,
		'debug-previous-game'
	);
});
