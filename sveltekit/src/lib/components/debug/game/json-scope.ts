// Tree-View node collection + JSON-scoping helpers for the Game debug tab's
...
import type { EnvelopeV2, GamePayload } from '$lib/types/scraper-v2';
...
export function buildTreeRoot(env: EnvelopeV2<GamePayload> | null): XNode {
 ...
 *                              e.g. selecting 'data.config.gametype' yields
 *                              { data: { config: { gametype: '...' } } }
 *                              regardless of whether the selected node is
 *                              a branch or a leaf. JSON.stringify renders
 *                              this with standard pretty-print.
 */
export function scopeAtPath(env: EnvelopeV2<GamePayload> | null, selectedValue: string[]): unknown {
	if (!env) return null;
