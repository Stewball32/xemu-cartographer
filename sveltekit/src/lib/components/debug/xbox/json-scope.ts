// Tree-View node collection + JSON-scoping helpers for the Xbox debug tab's
...
import type { EnvelopeV2, XboxPayload } from '$lib/types/scraper-v2';
...
export function buildTreeRoot(env: EnvelopeV2<XboxPayload> | null): XNode {
 ...
 *                              e.g. selecting 'data.xbe.title_name' yields
 *                              { data: { xbe: { title_name: '...' } } }
 *                              regardless of whether the selected node is
 *                              a branch or a leaf. JSON.stringify renders
 *                              this with standard pretty-print.
 */
export function scopeAtPath(env: EnvelopeV2<XboxPayload> | null, selectedValue: string[]): unknown {
	if (!env) return null;
