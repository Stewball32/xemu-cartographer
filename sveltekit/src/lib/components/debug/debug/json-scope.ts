// Tree-View node collection + JSON-scoping helpers for the Debug envelope
// tab's JSON view. Pure-TS (no Svelte/Skeleton imports) so the scoping
// rules can be unit-tested in isolation from the rendering layer.
//
// Sibling to xbox/json-scope.ts — same logic with DebugPayload typing so
// each tab's JSON view keeps its own envelope shape exact.
...
export function buildTreeRoot(env: EnvelopeV2<DebugPayload> | null): XNode {
 ...
 *                              e.g. selecting 'data.players' yields
 *                              { data: { players: [...] } } regardless of
 *                              whether the selected node is a branch or a
 *                              leaf. JSON.stringify renders this with
 *                              standard pretty-print.
 */
export function scopeAtPath(
	env: EnvelopeV2<DebugPayload> | null,
