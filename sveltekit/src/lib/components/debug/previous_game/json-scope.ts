// Tree-View node collection + JSON-scoping helpers for the Previous-Game
// debug tab's JSON view. Mirror of the Xbox tab's json-scope.ts (PR M6c
// xbox), specialized to the PreviousGamePayload shape so the type guards
// downstream stay tight.
...
export function buildTreeRoot(env: EnvelopeV2<PreviousGamePayload> | null): XNode {
 ...
 *                              e.g. selecting 'data.game.phase' yields
 *                              { data: { game: { phase: 'live' } } }
...
export function scopeAtPath(
	env: EnvelopeV2<PreviousGamePayload> | null,
