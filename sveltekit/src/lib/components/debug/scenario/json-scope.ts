// Tree-View node collection + JSON-scoping helpers for the Scenario debug tab's
...
//
// Duplicated from debug/xbox/json-scope.ts with the payload type swapped to
// ScenarioPayload — each tab owns its own scoping module so a future change
// to one tab's tree (e.g. flattening tag_defs entries) doesn't ripple into
// every other tab.

import type { EnvelopeV2, ScenarioPayload } from '$lib/types/scraper-v2';
...
export function buildTreeRoot(env: EnvelopeV2<ScenarioPayload> | null): XNode {
 ...
 *                              e.g. selecting 'data.fog.max_density' yields
 *                              { data: { fog: { max_density: ... } } }
...
export function scopeAtPath(
	env: EnvelopeV2<ScenarioPayload> | null,
